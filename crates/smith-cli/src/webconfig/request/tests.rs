...
fn get(target: &str, extra: &[(&str, &str)]) -> Result<Request, Refusal> {
    guard().admit(&head("GET", target, extra), "")
}

fn post(target: &str, extra: &[(&str, &str)], body: &str) -> Result<Request, Refusal> {
    let mut headers = vec![
        ("X-Smith-Token", TOKEN),
    ...
    guard().admit(&head("POST", target, &headers), body)
}
...
#[test]
fn the_page_without_a_token_is_refused() {
...
#[test]
fn an_api_call_without_the_header_token_is_refused() {
    assert_eq!(
        get("/api/state", &[]),
        Err(Refusal::Forbidden("token")),
...
fn a_request_naming_another_host_is_refused() {
    ...
    assert_eq!(guard().admit(&raw, ""), Err(Refusal::Forbidden("host")));
...
fn anything_off_the_route_table_is_a_not_found() {
    ...
    assert_eq!(guard().admit(&raw, ""), Err(Refusal::NotFound));
}
